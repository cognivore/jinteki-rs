...
fn the_doc_comment_and_the_data_carry_the_same_printed_text() {
    ...
    assert_eq!(checked, 179, "one check per card DEFINITION (Hedge Fund is defined but not listed; Gemilang Arena is Nebula's back face; Ken Tenma is CR 1.5.4a's pile; unlisted.rs is what no deck lists; identities/ is the CR 1.5.4a queue)");
}
