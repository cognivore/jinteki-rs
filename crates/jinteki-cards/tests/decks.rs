...
fn both_decks_build() {
    ...
    assert_eq!(cards.len(), 68, "both decks, one entry per distinct card (Hedge Fund is not in the printed Gauntlet list), plus CR 1.5.4a's pile");
    ...
    assert_eq!(pile_named("andromeda").unwrap().len(), 18);
...
fn the_doc_comment_and_the_data_carry_the_same_printed_text() {
    ...
    assert_eq!(checked, 139, "one check per card DEFINITION (Hedge Fund is defined but not listed; Gemilang Arena is Nebula's back face; Ken Tenma is CR 1.5.4a's pile; unlisted.rs is what no deck lists; identities/ is the CR 1.5.4a queue)");
}
