...
fn mezzies_decks_are_honest_as_far_as_they_go() {
    ...
    for (key, expect) in [("mezzie_asa", 19usize), ("mezzie_valencia", 10usize)] {
...
fn the_doc_comment_and_the_data_carry_the_same_printed_text() {
    ...
    assert_eq!(checked, 249, "one check per card DEFINITION (Hedge Fund is defined but not listed; Gemilang Arena is Nebula's back face, Ascending to Orbit is Earth Station's; Ken Tenma is CR 1.5.4a's pile; unlisted.rs is what no deck lists; identities/ is the CR 1.5.4a queue; mezzie_asa.rs is 6 of Mezzie's ice, 7 of her assets and 4 of her operations, mezzie_valencia.rs is Zer0 and three programs)");
}
...
fn every_definition_is_reachable_from_all_cards() {
    ...
    assert_eq!(checked, 249, "one reachability check per card definition");
}
