...
fn mezzies_decks_are_honest_as_far_as_they_go() {
    ...
    for (key, expect) in [("mezzie_asa", 8usize), ("mezzie_valencia", 6usize)] {
...
fn the_doc_comment_and_the_data_carry_the_same_printed_text() {
    ...
    assert_eq!(checked, 234, "one check per card DEFINITION (Hedge Fund is defined but not listed; Gemilang Arena is Nebula's back face, Ascending to Orbit is Earth Station's; Ken Tenma is CR 1.5.4a's pile; unlisted.rs is what no deck lists; identities/ is the CR 1.5.4a queue; mezzie_asa.rs is 6 of Mezzie's ice, mezzie_valencia.rs defines none yet)");
}
