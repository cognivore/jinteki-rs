...
| bucket | tests | note |
|---|---|---|
| **ported and passing** | 60 | `tests/corpus.rs`, manifest-ratcheted (`dp7c_odometer`) |
| **blocked on the card layer** | 3479 | card tests; unblocked card by card, in frequency order |
...
skip-trash,nothing-to-trash}`, `infiltration-{gain-2,expose}` `rashida-jaheem-when-there-are-enough-cards-in-r-d`, `neural-emp` and
`daily-casts`. **38 real cards** live in
