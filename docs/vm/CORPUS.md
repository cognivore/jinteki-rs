...
| bucket | tests | note |
|---|---|---|
| **ported and passing** | 68 | `tests/corpus.rs`, manifest-ratcheted (`dp7c_odometer`) |
| **blocked on the card layer** | 3471 | card tests; unblocked card by card, in frequency order |
...
skip-trash,nothing-to-trash}`, `infiltration-{gain-2,expose}` `rashida-jaheem-when-there-are-enough-cards-in-r-d`, `neural-emp`,
`daily-casts`, and from W17 `global-food-initiative`, `merger`,
`project-beale`, `fan-site`, `clot`, `seamless-launch`, `closed-accounts` and
`corporate-troubleshooter`. **45 real cards** live in
