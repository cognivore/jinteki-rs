...
- **Priority decks: 66/66 cards complete**, 0 printed sentences still
  unsayable (`cargo test -p jinteki-cards --test decks -- --nocapture`,
  ratcheted by `the_gap_list_is_measurable_and_honest`). Cards 51–66 are
...
- **Identity queue: 70/150** (`docs/vm/IDENTITY-QUEUE.md`, the list and the
  ...
  a pile. Runner Criminal 17/22, Shaper 10/21, Anarch 10/19. **Every complete
  Criminal is enlisted** in `cr::ANDROMEDA_PILE`, so `priority_decks()` is 66
