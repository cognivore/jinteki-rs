...
- **DP-7b: 894/1420** distinct rules cited (63.0%, the traceability test's
  own print); traceability test fails on any cited id absent from
  `docs/rules/cr-index.json`
- **Priority decks: 71/71 cards complete**, 0 printed sentences still
  unsayable (`cargo test -p jinteki-cards --test decks -- --nocapture`,
  ratcheted by `the_gap_list_is_measurable_and_honest`). Cards 51–71 are
...
- **Identity queue: 131/150** (`docs/vm/IDENTITY-QUEUE.md`, the list and the
  ...
  a pile. Runner Criminal 22/22 (COMPLETE), Shaper 19/21, Anarch 15/19,
  Neutral 3/3, Apex 1/1, Sunny 1/1, Adam 0/1; Corp Haas-Bioroid 17/19,
  Jinteki 18/21, NBN 17/19, Weyland 15/19, Neutral 3/4. **Every complete
  Criminal is enlisted** in `cr::ANDROMEDA_PILE`, so `priority_decks()` is 71
  ...
  Every identity the queue still has open is a recorded blocker or a card
  whose back face this repo has no printed text for.
...
|---|---|---|---|
| `-` | W23z | **one identity, and the number of tags the Runner is CONSIDERED to have.** NBN 17→18/19 (queue 132→133/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can (1.5.4b) — so the deck odometer is unchanged at 71/71. **`StaticDecl::ConsideredTagsMod { delta }`** is 10.5.2's number as a declaration: "the Runner is considered to have 1 additional tag (even if they have 0)" modifies what a tag reader SEES, never the counters, and which readers is settled the way the blocker entry said — `Quantity::RunnerTags`, `TriggerRequirement::RunnerTagsAtLeast` (10.5.2's "tagged", so Harishchandra Ent. opens the grip during Acme's encounter) and 5.2.6g's trash-a-resource gate read `Vm::considered_runner_tags`; 5.2.6e's remove-tag action, `Instruction::RemoveTags` and `Cost::remove_tags` (payability and payment both) stay on `st.runner.tags`, because a tag nobody has cannot be removed. **The break is deviation 2b's, held dynamically**: gathering the considered number asks every carrying static's 9.3.7a stated condition (`StaticCond::StateRequirement`), a stated condition can BE `RunnerTagsAtLeast` (Harishchandra), and the gather's `abilities_of` walk runs the 9.12.1 pipeline, whose gather evaluates `SelfStrength` quantities that can be `Quantity::RunnerTags` (Resistor) — two mouths, one a quantity, so a per-atom shallow match would have had to shadow `eval_quantity` whole; instead a latch makes any tag reader reached from INSIDE the gather answer the REAL count, and every reader outside it the modified one (so Resistor grows during Acme's encounter, and Harishchandra's grip opens); deviation 77 records the residue. The scope is the two filter atoms the entry named: `TargetFilter::IsEncounteredIce` (6.5.1 — `TargetSpec::EncounteredIce`'s description half, read from the encounter record so 6.1.4's uninstalled encounter qualifies) and `TargetFilter::OutermostIceOfItsServer` (6.2.1/6.2.2 — the last ice of its OWN server's innermost-first sequence, server-generic on purpose: AgInfusion's "outermost ice of the chosen server" is this atom plus a server criterion, and its blocker entry now says so). The stated condition rides `declares_while` + `board_has` over both atoms. EDSL: `considered_additional_tags(n)`, `the_encountered_ice()`, `outermost_ice_of_its_server()`. `testkit::considered_tag_probe` (an encounter-met conditional requiring N tags and paying `Quantity::RunnerTags` — both modified readers in one card) and `testkit::tag_shedder` (a remove-1-tag paid cost — the real-count side). Cards: **Acme Consulting: The Truth You Need — COMPLETE**, 4 behaviour tests: the outermost encounter of a two-ice server reads 1 with 0 real tags and the real count never moves; the inner encounter reads 0 (the unrezzed outer ice still holds the outermost position); no encounter reads exactly the real count; and 1 real tag + the qualifying encounter reads 2 while the shedder's cost still removes only the real one. Deleted from the queue's blocker list: "nothing modifies the NUMBER of tags the Runner is considered to have" | 247 |
| `-` | W23y | **no new identity — the ordinal's re-reads of the past, made honest (audit 2026-08-05 finding 1).** Queue unchanged at 132/150, deck odometer unchanged at 71/71. 9.6.5c's ordinal scan re-asks the WHOLE condition of every earlier change this turn, and three conditions answered part of it off the board AS IT NOW STANDS — so mid-turn state movement rewrote which occurrences had been "the times". Three facts of the occurrence's own moment move onto their records (the A Teia record-vs-state rule, twelfth confirmation of the field-on-record shape): **`GameChange::RunDeclaredSuccessful` gains `subroutine_resolved`** (9.8.7 asked of the run's own history window at the declaration, which is closed by the time any rescan asks) — Ryō "Phoenix" Ōno's "after a subroutine resolved during that run" stops being a `requires`, which the rescan SKIPS, so a plain successful run was spending the turn's one time the printed sentence says it never met (the audit's first WRONG); `TriggerCond::MakesSuccessfulRun` gains `after_subroutine_resolved` and reads the record. **`GameChange::EncounterEnded` gains `ice_advanced`** (1.18.2 at 6.5.10's moment) — Builder of Nations read the ice's counters as they stand NOW, so counters placed later re-counted a plain encounter as advanced (suppression) and counters leaving with the ice un-counted a real one (double-fire), the audit's second WRONG; `TriggerCond::EncounterEnds` gains `with_advanced_ice`, its `criteria` keeping only characteristics, which travel with the card. **`GameChange::IcePassed` gains `rezzed`** (8.1's status at the pass) — Architects of Tomorrow's "a **rezzed** piece of bioroid ice" was a criteria read, so an ice passed unrezzed and rezzed later the same turn became retroactively one of "the times" (the audit's DUBIOUS); `TriggerCond::IcePassed` gains `rezzed`, the bioroid half staying criteria. **The doctrine the fix settles, written on `MakesSuccessfulRun.requires`: a `requires` is 9.6.5c's stipulation about the STATE, gates the occurrence being considered, and sits OUTSIDE what the printed ordinal counts — the Jamie "Bzzz" Micken parse (W23d), whose leading "if" does not change which installs are the times, stands and is now by design rather than by omission (the audit's fourth sighting, reclassified correct). A stipulation INSIDE the count must ride the occurrence's record.** EDSL: `makes_successful_run_after_subroutine_resolved()`, `encounter_with_advanced_ice_ends()`, `passes_rezzed_ice_matching(…)`; `makes_successful_run_if`, `encounter_with_ice_matching_ends` and `advanced()` retired with their only users. `testkit::run_ends_advancer` (a fixed-ice advancement placer met by a run ending, for moving counters mid-turn without choreography). Tests: 3 behaviour tests, one per record — a subroutine-less successful run before a qualifying one, the same ice passed unrezzed then rezzed in one turn, counters placed on a passed ice between two runs — each pinned by a change-log POSITION assert (the effect follows the SECOND run), because the counts alone cannot say which occurrence fired: a matcher that drops the stipulation entirely fires on the first run and passes every count. Each of the three record reads was reverted in turn to confirm its test fails without it. New deviation 76 records the residue: characteristic criteria in ordinal re-reads go through the CURRENT 9.12.1 pipeline, exact for printed characteristics and wrong for granted-with-duration ones, which no ordinal in the corpus yet reads | 247 |
| `-` | W23x | **one identity, and the ability an install was MADE BY.** Anarch 15→16/19 (queue 131→132/150); nothing joins `cr::ANDROMEDA_PILE` — Topan is Anarch, and 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 71/71. **`GameChange::CardInstalled` gains `by_ability_of: Option<ObjectId>`** — the card whose PRINTED ability's resolution performed the install, one more fact about the MOMENT beside `from` and `to` (the A Teia record-vs-state rule: the frame that made the install is popped long before any condition is scanned), read off the innermost ability frame at step 8.5.16f, the exact line where W23l already records `installed_cards`. A rules frame attributes nothing — 5.2.6d/5.2.7d's basic action is the player's, not a card's, and its `AbilityRef` convention (`index: usize::MAX`) marks it; NOTE the gate is the record's honesty rather than anything a card yet observes, because a rules frame's ref names the card being ACTED ON and would fail the source comparison anyway. 8.8.4b's swap records `None` too: the card "becomes installed" explicitly WITHOUT the 8.5.16 procedure, so no ability installed it. **`TriggerCond::CardInstalledByAbilityOfSource`** is the queue's missing word said exactly: "When you install that card, …" is a conditional whose occurrence happens while its OWN ability's earlier instruction is resolving, so `CreateDelayedConditional` cannot carry it (it resolves after the install, and 9.6.13 would have it wait for the NEXT one) and a plain installs-a-card condition would be met by every install the player makes — what the sentence stipulates is WHOSE ability made the install, compared against the source the way every other "this card" condition is. **The `InstallCard` announcement now refuses non-installable types** (8.5.1/8.5.3, the same sentence `could_install_found_card` already reads for searches) unless the install is a facedown one (8.1.4a leaves no type to judge by) — Topan's untyped "Install 1 card from your grip" is the first faceup install description that names no type, and without the retain an event in the grip was a candidate (1.15.3). EDSL: `installs_that_card()`. Cards: **Topan: Ormas Leader — COMPLETE**, 2 behaviour tests — the ability's install (a 3-cost program for 1[credit]; the install recorded BEFORE exactly one meat damage; the event never a candidate; the action offered once a turn) and the basic action's install meeting the condition not at all. Both widenings (the cond's comparison, the announcement retain) were reverted in turn to confirm a test fails without each. Deleted from the queue's blocker list: "a condition cannot be met by an earlier instruction of its OWN ability" | 247 |
| `-` | W23w | **one identity, and who a damage was DONE BY.** Jinteki 17→18/21 (queue 130→131/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can (1.5.4b) — so the deck odometer is unchanged at 71/71. **`GameChange::DamageSuffered` gains `responsible: Side`** — 10.4.1 splits responsibility on the printed VERB (a card that "does" damage: its side and the source; one that directs the Runner to "suffer": the Runner and the source), and the split is a fact about the OCCURRENCE, recorded where `do_damage_selecting` already held it (the A Teia record-vs-state rule: nothing later could answer it). **`TriggerCond::RunnerSuffersDamage` gains `responsible: Option<Side>`** — "whenever **you do** damage" (AU Co.) is a third stipulation about the same occurrence beside `kind` and `trashed_a_card`, not a condition of its own; `None` is every sentence that existed before. **Audit finding 3 closed by the same word: Argus Security's suffer branch was `meat_damage(Corp, 2)`** — `testkit::argus_like` had the Runner responsible all along, with 10.4.1 quoted; the card now agrees (`meat_damage(Runner, 2)`), its doc comment stops citing 10.4.2 for a claim 10.4.1 owns, and its behaviour test asserts the record. **`TriggerCond::CardTrashed` gains `at_least_one: bool`** — 9.12.2a's plural noun ("you trash **1 or more cards** from HQ") as content beside `from_zone` and `by`, read by `trigger_per_event` (which became a match): the singular sentence (Nuvem SA) is met once per card, the plural once per occurrence GROUP, and only the noun's number tells them apart. **`trigger_per_event` recurses through `AnyOf`**, so an either-of sentence keeps each half's own number — the damage half was one aggregated occurrence to begin with (10.4.3/9.12.2c). Blocker's claims checked against the code (the standing lesson): the "Corp-side batched trash condition that does not exist" was three of `CardTrashed`'s existing fields plus one bool. **One announcement repair for "the rest": `announce_position`'s AddCardsToHand arm** (the 1.15.2e reached-for ceiling, `up_to: true`) **no longer captures a `Quantity::Count` spec** — a count the DESCRIPTION itself supplies ("add **the rest** to HQ" = looked-at ∧ ¬`AmongEarlierTargets`, the words Steve Cambridge landed) leaves no number to reach for, so the announcement is the whole set and MANDATORY (`up_to: false, min: count`; the behaviour test pins that shape, so answering with fewer is not an answer). Extending 540fe7a's forced answer to exact full-set choices was tried and REVERTED: nine behaviour tests observe those announcements' candidate lists as their assertion surface, and a full-set prompt is renderable — redundant is not unanswerable, which is the line 540fe7a actually drew. EDSL: `does_damage(side)`, `trashes_at_least_one_card_from(side, zone)`; `testkit::suffer_damage_button` (Stimhack shape). Cards: **AU Co.: The Gold Standard in Clones — COMPLETE**, 2 behaviour tests — four occurrences worth two counters (a 2-point damage: one; a two-card HQ trash event: one; a SUFFERED damage and a Runner's trash of an HQ card: none), and the turn-begins spend over pay/decline/unpayable (counters 2→0 exactly on pay; the trash's choice of 3 announced, "the rest" moved with nobody asked; 1 hosted counter offering nothing). Each of four widenings (the responsible read, the at_least_one per-event read, the AnyOf recursion, the Count exclusion) was reverted in turn to confirm a test fails without it. Deleted from the queue's blocker list: "neither half of AU Co.'s first line is a condition yet". The headline odometers above also catch up with W23v (queue 130, deck 71, pile 21, Criminal complete), which its row recorded and the header missed | 247 |
| `-` | W23v | **one identity, and the OTHER card — the one a later choice did not take.** Criminal 21→22/22, the faction COMPLETE (queue 129→130/150); **Steve Cambridge joins `cr::ANDROMEDA_PILE`**, so the deck odometer moves 70→71 (pile 20→21) and Rebirth's Criminal choice widens again. Three words, all in the 1.15 family. **`Instruction::ChooseCards { targets }`** is 9.11.4c said as an instruction: a sentence that only chooses usually merges into the sentence that acts on the choice (Tinkering, SSO Industries) — here the following sentence's TWO halves act on DIFFERENT subsets of it, so no half can own the announcement, and the choosing sentence stands as the combined's first half, announcing its position like any other and resolving to nothing at all (no atom — inside a `Combined` the half is applied as the nothing it is). **`TargetFilter::AmongEarlierTargets`** is "1 of THOSE cards" — 1.15.4's plural read as a DESCRIPTION a later announcement picks from, the `ability_targets` twin of `AmongTriggeringCards`, fixing cards by identity so 1.15.2c lifts. It forced an honest look at `announcement_for_at`'s cross-announcement retain: 1.15.2e restricts repeats "for **each announcement**", not across the instruction, so the retain is a conservative over-application that a "2 different pieces of ice" spec relies on — it stands, EXCEPT where a criterion points INTO the earlier announcements, where it would leave the printed sentence describing nothing. **`TargetSpec::EarlierTargetsExceptLatest`** is "the other card": the current instruction's earlier announcements minus its latest one, a set difference over the announcement record rather than a zone question (1.15.4 lets the sentence act on the card wherever the other halves moved things) — which is also what makes the one-card-heap ruling fall out: 1.15.2e announces the one card, the Corp's pick names the same object, the difference is empty and 1.15.3 adds nothing. Two `Combined` repairs rode along: the in-place whitelist widened to `RemoveCardsFromGame` (its arm asks no Decision) with 1.14.5's wrapper peeled — who chooses does not change what resolving does — and the Inline sub-wrap now carries each half's OWN announcements (`span_announcements_owed`, the spans-only sibling of `announcements_owed`), closing a latent defect no card had reached: with two announcing halves, an in-place half's `Choose` read the flat union and would have acted on every half's targets at once. Cards: **Steve Cambridge: Master Grifter — COMPLETE**, 2 behaviour tests — a three-card heap over accept and decline (the Runner's candidates are the whole heap, the Corp's are EXACTLY the two announced cards and never the third; the pick removed, the other card in the grip — directly, never via the removed-from-game zone, which is the assert that catches the union defect; the third card untouched; the second HQ run of the turn not offered; the decline moving nothing) and the one-card heap giving the Runner nothing. Each of five widenings (the retain lift, the slice, the spec, the filter, the whitelist) was reverted in turn to confirm a test fails without it — the slice needed the stronger assert first, because the add half was quietly rescuing the wrongly-removed card. Deleted from the queue's blocker list: "'the other card' cannot be said"; a new entry states what AU Co.'s FIRST line still wants (a responsible side on `GameChange::DamageSuffered`, and a Corp-side batched trash-from-HQ condition beside `RunnerTrashesAtLeastOneCorpCard`) — its "the rest" now waits on nothing | 247 |
| `-` | W23u | **one identity, and the announcement a combined sentence owes its own halves.** Weyland Consortium 14→15/19 (queue 128→129/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`Instruction::Combined` is `Contained::Inline`, not `Deferred`.** 9.11.3 makes an "X and Y" sentence ONE instruction, and 1.15.2 scopes an announcement to "the instruction" that requires it — so a half that chooses its targets was having them announced in the wrong place: the splice put the choice AFTER the merged half had resolved, and 1.15.4's back-reference in the merged half read nothing at all ("add 1 rezzed card to HQ and gain credits equal to **its** rez cost" gained 0). Classified inline, the halves announce before any of the sentence resolves — the derived machinery of W22a needed no new code, only the honest classification — and a §9.2.2e procedure half stays out of the walk (9.11.4b splits several plays/installs into instructions of their own; the same fork `DeclineableChoice` already makes). **`EffectAtom` grows `combined_half`**, stamped by the merge, so resolution applies each half's own atoms at the half's own place in printed order instead of walking the merged set blind — which is also what keeps two same-class halves from double-applying if a card ever prints them. The splice narrows rather than dies: a half the atoms cannot carry is applied as part of THIS instruction when its resolution asks no Decision (`AddCardsToHand`), and spliced when it does (a sabotage's 10.12.2 division needs the frame machinery an instruction gets) or when a spliced half precedes it (Multifarious Marketeer still reveals before adding to the grip). Every other `combined(` card in the corpus keeps its exact path — the 232-test behaviour suite passed unchanged. **`Quantity::RezCostOfEarlierTarget { nth }`** is `RezCostOfEncounteredIce`'s pointing twin: 1.15.4's back-reference asked for a number, read off the announced card's PRINTED cost (1.16.4a) at imminence, while the sentence's first half has yet to move the card — the number travels with the card, so the read survives the move it precedes. EDSL: `rez_cost_of_earlier_choice(nth)`; `combined`'s doc comment stopped saying "same class only" (the guide already said the opposite — Snare! is the case it exists for) and stopped calling the structural drop current (it was fixed in W17d). Cards: **Blue Sun: Powering the Future — COMPLETE**, 1 behaviour test over accept and decline — two rezzed cards with different printed rez costs and the gain matching the CHOSEN one (the "its" that was unreadable), the unrezzed asset absent from the candidates (8.1.2 bites at the announcement), the chosen ice in HQ with the other card unmoved, and the decline moving and gaining nothing. Each of the three widenings (the Inline classification, the quantity, the in-place application) was reverted in turn to confirm the test fails without it. Deleted from the queue's blocker list: "a sentence joined by 'and' cannot refer to the card its own other half chose"; the AU Co. entry restated — its "and" now waits on nothing, only on "the rest" | 247 |
| `-` | W23t | **one identity, and the servers OTHER than the one an occurrence names.** Jinteki 16→17/21 (queue 127→128/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`GameChange::CardInstalled` gains `to: Zone`.** The record carried where the card came FROM and never where it went, and 8.5.16b's destination is a fact about the MOMENT of the install for the same reason `CardTrashed::was_zone` is: the card can have moved on before any condition is scanned. It matters most for 9.6.5c's ordinal, whose scan re-asks the whole condition of every EARLIER change this turn — an install into a remote whose card is scored or trashed later the same turn has still spent the offer, which a reading of the card's current zone would hand back. 1.13.12 puts a hosted card in its host's zone, so a card hosted at install time records the server 4.6.6b puts the host in; 8.8.4b's swap-in records the position the leaving card occupied. **`TriggerCond::CardInstalledBy` gains `into_remote_server: bool`** (4.6.6b + 4.6.8) — "you install a card **in the root of or protecting a remote server**" stipulates nothing about the card and everything about the destination, so it is neither `of_types`/`of_subtypes` nor a description of the object: 4.6.6b makes "in the root of or protecting" ONE location word, and 4.6.8 is what lets it draw the remote/central line. `false` is a sentence making no such stipulation, which is the eight conditions that already existed. **`InstallDest::DeclaredByInstallerInAnotherRemoteServer`** is `DeclaredByInstallerInServerOfTriggeringCard`'s 1.15.4 comparison INVERTED and narrowed by 4.6.8 the way W23s's `DeclaredByInstallerInRemoteRoot` is — the same `retain` on the same offered list, keeping both of 4.6.6b's halves so the installer still declares the server AND whether the card goes in its root or protects it. 8.5.2a's brand-new remote qualifies without comparison (a server that does not exist yet is not one anything is in) and is ruled out by 4.6.8f alone. With no card named by the occurrence, or one in no server, "another" has nothing to be other than, no destination is identified, and 8.5.14 stops the install — the same answer the un-inverted variant gives. EDSL: `installs_a_card_in_a_remote_server(side)`, `remote_server_limit(n)`, `this_turn()`. Cards: **A Teia: IP Recovery — COMPLETE**, 3 behaviour tests — a piece of ice installed protecting HQ that fires nothing and spends no ordinal, then one protecting a remote that fires the ability and is offered exactly ONE position out of the five a piece of ice could otherwise take (the same remote is not "another", the three centrals are not remote, and 4.6.8f's limit of 2 has already ruled out the new one), with 8.5.11a's 1[credit] for the ice already there waived by 1.16.5c; a 0/1 agenda whose (S) option is withheld for the rest of that turn and back on the next; and an install into a remote whose agenda is SCORED out of it before a second install in a remote, which still gets no second offer. Each of the three widenings was reverted in turn to confirm the tests fail without it. Deleted from the queue's blocker list: "a destination cannot be narrowed to the servers OTHER than one" | 247 |
| `-` | W23s | **one identity, and a "cannot" said about ONE card for a span longer than a turn.** Jinteki 15→16/21 (queue 126→127/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`LingeringSpec::Prohibit` / `Payload::Prohibited { target, actions }`, with `ProhibitedAction::{Score, Rez}`.** `StaticDecl::CannotScoreMatching` says 1.2.2's "cannot" about the agendas a DESCRIPTION reaches, which is how Clot's "during the same turn they installed that agenda" is written — and "you cannot score or rez **that card**" is the other kind: the card is fixed when the ability that said it resolved, so a second copy installed later is untouched and no description could tell them apart. That makes it a 9.10.1 lingering effect and not a static declaration, which is also what gives it a DURATION. Which acts are forbidden is content on the one atom (§12 rule 2): a sentence naming two of them is one prohibition, and A Teia's shorter sentence is the same atom with a one-element list. The position DESCRIBES its objects rather than targeting them, exactly as `CannotUseAbilitiesOf` does, so nothing is announced — "that card" is `TargetSpec::InstalledByThisAbility`, which 8.5.16f records and 1.15.2 never saw. **`Vm::rez_prohibited`, asked in three places.** 1.2.2 gives the "cannot" precedence over every ability that would rez, so the (R) options of a paid ability window and the approached-ice one are both withheld, and `rez_card_inner` — the single funnel every rez goes through, 8.1.2's procedure — refuses one an ability directs. `score_prohibited` already existed and gained the same read. **`Duration::UntilNextTurnBeginsOf { side, after }` + `WantedDuration::UntilNextTurnBeginsOf(Side)`.** `WantedDuration` stopped at `ThisTurn`, and this span is longer by exactly one turn: it runs through the rest of this turn, through the whole of the opponent's, and ends the moment the named side's next turn begins. 5.1's alternation is why it is read as "the first LATER turn whose active player is that side" rather than counted two forward — that reading is also right for an effect created during the OTHER player's turn. `LingeringEffect::expired` therefore takes the active side beside the turn sequence number; 9.10.4 has nothing to bind, since a turn is always in progress. **`InstallDest::DeclaredByInstallerInRemoteRoot`** is 8.5.16b's declaration narrowed the way `DeclaredByInstallerInServerOfTriggeringCard` is, by a `retain` on the same offered list: 4.6.8's remote servers and 4.6.6b's root half of them, with 8.5.2a's brand-new remote among the options because 4.6.8d creates it at install step 8.5.16e like any other. A card that could occupy no such root leaves no destination to identify and 8.5.14 stops the install, which is the same answer Asa Group's narrowing already gives. EDSL: `cannot_be(target, &[…], duration)` and `until_next_turn_begins_of(side)`. Cards: **Saraswati Mnemonics: Endless Exploration — COMPLETE**, 2 behaviour tests — an upgrade installed into an existing remote's root (with every central root and every protecting position among what 8.5.16b would otherwise have offered), which then cannot be rezzed at cost 0 while the asset sharing its root is offered throughout, is still not offered during the Runner's run on that server, and is offered again at the first window of the Corp's next turn; and a 1/1 agenda whose advancement requirement the ability's own counter meets, with the (S) option withheld for the rest of that turn and back on the next. Each of the four widenings was reverted in turn to confirm the tests fail without it. Deleted from the queue's blocker list: "a prohibition cannot name one card for a stated span, and none of them names a REZ" — restated for A Teia, which now waits only on a destination narrowed to the servers OTHER than one, and on a description of WHERE an install went riding on `TriggerCond::CardInstalledBy` | 247 |
| `-` | W23r | **one identity, and the cards ONE occurrence trashed.** Haas-Bioroid 16→17/19 (queue 125→126/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`change::card_named_by` becomes `cards_named_by`, and `DamageSuffered` is in it.** 1.15.4's naming was already read in the plural (`triggering_cards`, W23k), but only across the several CHANGES of one occurrence group: each change could name at most one card, and the function said in so many words that damage's trashed set therefore named none. 10.4.3 is why that was the wrong shape — more than 1 damage of a type trashes its cards SIMULTANEOUSLY, one change carrying all of them — so a single change naming several is 9.12.2c at work and not a defect to be refused. `TargetFilter::SameNameAsTriggeringCard` now reads that plural: an occurrence naming one card leaves a one-element list and asks exactly the question The Foundry always asked, and the widening is load-bearing (removing it fails the two-core-damage half of the behaviour test). **`TriggerCond::RunnerSuffersDamage` grows `trashed_a_card`.** The queue's blocker entry looked for a record of "the Runner trashes a card FOR brain damage" beside `CardTrashed` and `SelfTrashedByDamage` and found neither — but 10.4.2a/b make the trash the damage procedure ITSELF ("for each point of damage suffered, the player responsible for the damage trashes 1 randomly-chosen card from the grip"), so the trash and the damage are ONE occurrence and the sentence is a 9.6.5c stipulation about it, not a condition of its own. 10.4.2c is what makes "brain damage" core damage, and the printed "the Runner trashes" is the older wording of 10.4.2b's responsible player. The stipulation is written because the card writes it and not because a case waits on it: the only damage that trashes nothing is damage against an empty grip, and 1.7.2b flatlines the Runner for that — the doc comment says so rather than implying a board could show it. **`Instruction::ShuffleDeck { side }`**: 4.2.3 keeps a deck's order "except when a player is explicitly directed to manipulate the cards in a deck", and "then, they shuffle their stack" is that direction with nothing moving into the deck — the half `ShuffleCardsIntoDeck` does after its move and 8.7.3 does after a search, said on its own because this sentence says only that half. **Nothing new was needed for the sentence the blocker entry called unwriteable**: "all copies of that card (installed, in the heap, stack, grip, or any other location)" is `SameNameAsTriggeringCard` beside `InAnyLocation`, whose own doc comment already quoted this card — 1.15.2c's parenthesis lifting the installed-cards default is exactly what the printed parenthesis is for. Cards: **Chronos Protocol: Haas-Bioroid — COMPLETE**, 2 behaviour tests (one core damage against a one-card grip, with copies in the rig, the heap and twice in the stack, all four removed with the trashed card itself and the stack left shuffled; and the kind/plurality pair — a net damage trashing the same cards reaches nothing, one core damage reaches the copy of whichever card 10.4.3's randomiser took, and two core damage reach the copies of both). Deleted from the queue's blocker list: "nothing is met by trashing a card TO PAY for damage" | 247 |
| `-` | W23q | **one identity, and the one instruction before a run succeeds.** Runner Anarch 14→15/19 (queue 124→125/150); nothing joins `cr::ANDROMEDA_PILE` — 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`EffectClass::DeclareRunSuccessful`, and `Instruction::InitiateRun` grows `if_would_be_successful`.** 9.9.3d makes a "would" trigger condition relevant when the imminent instruction's EXPECTED EFFECTS meet it, and 6.9.5a's declaration had none of its own: it fell to the catch-all structural atom, which is what every other step in the run carries, so no condition could tell the Success Phase apart from the Approach Server Phase. The atom carries no value — 6.7.1's declaration is not a number — and exists for that one purpose. 9.9.2 is the other half and is why the atom is REMOVED when a Crisium-class static prohibits the declaration: a run that cannot be declared successful would not be, and the expected effects are the place that has to say so. The prohibition was already read at resolution; reading it at imminence too is what stops the ability being offered, paid for, and resolved against a declaration that never comes. **The clause rides on the instruction, not on the card.** "Run Archives. **If that run would be declared successful**, …" is two sentences and so two instructions (9.11.3), but the second cannot be the ability's second instruction: 6.1's run is a nested timing structure and everything after `InitiateRun` resolves once the run is over. It goes exactly where 6.7.4's "If successful" clause of an initiating effect goes — `RunCtx` — and for one further reason 6.7.4 does not have: 9.9.4b gathers pending interrupts by scanning the abilities PRINTED on objects, and this one is printed on none, so `pend_would_be_successful` is a second pending site beside that scan. 6.7.4a's server tie is deliberately NOT applied to it: that rule is stated about "If successful" abilities, and this sentence names the RUN, which is what holds the clause. `TriggerCond::WouldDeclareRunSuccessful` rides on the definition the pending site constructs rather than being checked once at the door, because 9.9.4c re-asks relevance of every pending instance before the window offers it. Cards: **Omar Keung: Conspiracy Theorist — COMPLETE**, 2 behaviour tests (the run announced on Archives and succeeding on HQ, with a rezzed end-the-run ice on HQ as the control that 6.1.2d never moves the Runner past; and the same run into a Crisium-class Archives, where the interrupt is `Reply::Forbid`den and the breach still happens — on Archives). Deleted from the queue's blocker list: "nothing is met by a run that WOULD be declared successful" | 247 |
| `-` | W23p | **one identity, and the two ends of "you finish resolving".** Weyland Consortium 13→14/19 (queue 123→124/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`GameChange::CardPlayResolved` grows `by`, and `TriggerCond::CardPlayResolved` is the general form of `SelfPlayResolved`.** 8.6.7h has always been recorded; what it could not answer is WHO finished resolving the card and WHAT the card was, because the self-view fixes the card by identity (1.15.4) and says nothing else about it. The player is 8.6.2's and is read off the occurrence rather than off the card, for a reason step (g) settles: the operation has already been trashed by the time the condition is scanned, and 1.14.1's owner is a different question. The description is the shared filter vocabulary (§12 rule 5), asked of a card in Archives — which works because a description reads the card's own characteristics and those travel with it. **`GameChange::ActionCompleted` grows `action`, and `TriggerCond::ActionCompleted` reads it.** 5.2.2d is already the moment "conditions related to an action finishing" are met and 5.2.2a already puts it at the end of the action step; the queue's blocker entry looked for a record of an ability FINISHING and did not see that this one was it — `GameChange::AbilityUsed` is 9.1.6's other end and was the wrong twin. The identity is the same 5.2.5a/b `ActionTaken` carries, recorded here as well because the action step is over by the time the scan asks, exactly as `CardRezzed` carries the card type. A description reaches the CARD the action is an ability of (5.2.4); 9.1.3 makes a basic action's source a game rule, so no description reaches one and playing an operation with 5.2.6e meets the play half only. **Defect fixed, found by the first card ever met by an action finishing: `ActionCompleted` was recorded once per RE-ENTRY of the step's closing checkpoint, not once per action.** `checkpoint_and_react` pushes a frame when it opens a reaction window and the structure frame's phase is not advanced until that window closes, so the `StepPhase::Checkpoint` arm ran again — and recorded the completion again — for every ability that reacted to the action. `ActionsFinishedThisTurn` was the only reader and compares against a threshold, so one and two were indistinguishable to it; a condition MET by the record loops forever. The guard is 5.2.2a read off the log (10.2.1): an action must be completed before another is initiated, so `ActionTaken` and `ActionCompleted` alternate and the later of the two says which end this action is at. Also **`TriggerCond::CardTrashed` grows `from_zone` and 9.6.5c's `requires`** — the one zone a sentence names, which is not a narrowing of `installed_only` (8.1's play area is many zones) — and **`TriggerRequirement::ActiveTurnIs`** (9.2.1), which is how a span narrower than `OrdinalScope::Turn` is said: the ordinal counts inside whichever turn is being played, so "during each of **your** turns" states whose turn it must be as well. EDSL: `finishes_resolving_a_played_card`, `finishes_an_action_on`, `trashes_a_card_from`, `during_the_turn_of`. Cards: **Nuvem SA: Law of the Land — COMPLETE**, 3 behaviour tests — three operations in one Corp turn (three looks, not six, because the basic play action is no card's ability; two trashes, one of them declined; one payment); the same [click] ability on the same asset with and without the printed subtype, with a basic credit action after it as the control; and the same trash from R&D made once at the start of a Corp turn and once at the start of the Runner's. Deleted from the queue's blocker list: "nothing is met by an ability FINISHING" | 247 |
| `-` | W23o | **one identity, and a declaration stated about ONE occurrence and offered.** Jinteki 14→15/21 (queue 122→123/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`StaticDecl::SelectsDamageTrashes` grows `kinds`, `first_each_turn` and `optional`.** 10.4.3a's declaration modified the damage procedure for every damage, always, and could not be declined; the three things Chronos Protocol: Selective Mind-mapping's one sentence stipulates about it are all CONTENT on that one atom (§12 rule 2) rather than three atoms or three cards' worth of machinery. `kinds` is the damage type the sentence names — 10.4.2 lists three, and an empty list names none, so it reaches all of them, which is what a sentence saying plain "damage" (Titanium Ribs) means. `first_each_turn` is the printed ordinal, read from the change log at `st.turn_log_start` exactly as `StaticDecl::InherentCostMod`'s already is (10.2.1 open information) and deliberately NOT 9.3.6g's once-per-turn flag: 9.4.1 says a static ability never resolves, so it could never spend one. The count is taken before this damage's own `DamageSuffered` is recorded, so the damage being decided is never one of the earlier ones it is counted against — and a damage that was PREVENTED was never suffered, so it does not use the turn up. `optional` is the printed "**you may**", and where it is asked is the whole of it: the word governs the looking as well as the selecting, so the offer is a `DecisionSpec::OptionalEffect` put BEFORE the grip is named (`DecisionCtx::DamageSelectionOffer`), and only a Corp who accepts sees the candidates. Offering the grip as a `ChooseTargets` the Corp may answer with nothing would have shown them a hand 4.3.2 keeps hidden, for free, every turn. **Defect fixed on the same path: the selected damage trashed the cards as the CORP.** `DecisionCtx::DamageSelection` carried the kind and the amount across the suspension but not 10.4.1's responsible player, and the `answer` arm passed a literal `Side::Corp` — so a damage the RUNNER was responsible for (10.4.1's "if a card directs or allows the Runner to suffer damage") recorded the wrong trasher on every `CardTrashed` it made, which is what a "whenever the Corp trashes a card" condition reads. It is now carried on the context, where the kind and the amount already were. EDSL: `may_select_first_damage_trashes_each_turn(by, kind, count)` and `selects_damage_trashes(by, count)`. Cards: **Chronos Protocol: Selective Mind-mapping — COMPLETE**, 2 behaviour tests — the Corp presses one net-damage button twice in a turn and selects for the first only, and then the two halves the sentence states about which damage it reaches, with a meat damage offering nothing and a declined net damage naming no grip. Deleted from the queue's blocker list: "a declaration cannot be scoped to ONE occurrence, or offered"; deviation 55 narrowed to what is left of it | 247 |
| `-` | W23n | **one identity, and what a BASIC ACTION's credit is paid for.** Weyland Consortium 12→13/19 (queue 121→122/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **5.2.6f's 1[credit] now goes through `Vm::begin_payment`**: `CreditPurpose` already said what a payment is FOR — trashing a card, using a card's abilities (9.1.6a), a trace attempt's spend steps — but the basic advance action took its credit straight out of `st.corp.credits`, so there was no payment to carry a purpose and no 1.10.3c division for the payer to make. It is one `PaymentCont::BasicAdvance { card }` beside `BasicTrash`, read by `purpose_of` exactly as that one is; the click is still spent by `spend_click` as the action is taken (5.2.2), so only the credit half travels with the payment. The action's availability moves with it: `1.16.1b` withholds an action nothing can pay for, and what CAN pay is now asked once per advanceable card (`spendable_credits_for(Corp, Advancing(id))`) rather than once for the action, because a card allowing its credits for one class of payment answers about the card being advanced and not in general. **`CreditUse::AdvancingCards(criteria)` + `CreditPurpose::Advancing(obj)`**: 1.18.1 makes advancing "placing an advancement counter on a card by paying for it", so the card a restriction describes is the one the counter goes ON — the ordinary filter vocabulary (§12 rule 5), with 1.15.2c's default making a printed "ice" reach the installed pieces of ice. It is deliberately not `UsingAbilitiesOf` under another name: a basic action has no card whose ability is being used. **Defect fixed, found by the same question: `GameChange::CostPaid` named a card as the CAUSE of a basic action's payment.** 9.1.3 is that "the source of an ability is the card, counter, **or game rule** that originated it" and 9.1.1 counts the basic actions and the basic trash ability among the abilities, so neither has a card source — but `pay_cost_committed` recorded `Some(p.source)` unconditionally, and `Payment::source` for those two is only the card the action ACTS ON (kept there so 1.16.2e's alternate payments and 1.9.2's counter costs have something to be about). GameNET therefore paid its controller a credit whenever the Runner used 7.1.5's basic trash ability on a Corp card during a run, which is not a Corp card ability causing anything. `Vm::cause_of` is the one place that question is now asked. The advance's spurious `GameChange::CreditsLost` goes with it: 1.10.3b keeps spending and losing apart, and a payment is a SPEND, so the old code recorded one advance as both and a "spends **or** loses" condition said about the Corp would have been met twice by it. Nothing was meeting it twice yet — GameNET, the only card printing that sentence, says it of the Runner — which is exactly why the record could sit there wrong. EDSL: `credits_only_for_advancing(&[…])`. Cards: **Weyland Consortium: Because We Built It — COMPLETE**, 1 behaviour test over four plays — the same "you can advance this card" declaration on a piece of ice and on an asset, so 1.18.3 offers both and only the payment tells them apart; then a pool credit, which advances the asset after all and leaves the identity's credit alone; then both at once, which is 1.10.3c's division actually being put to the Corp. Deleted from the queue's blocker list: "the basic ADVANCE action does not pay through a payment" | 247 |
| `-` | W23m | **one identity, and the INNERMOST end of a server's sequence of positions.** Jinteki 13→14/21 (queue 120→121/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can, since 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`InstallDest::InnermostProtectingAttackedServer`**: §6.2.2 states three position rules and `InstallDest` carried two of them — 6.2.2a's outermost (`Protecting`, which 8.5.2d makes the default of every Corp install) and 6.2.2c's directly-inward (`InwardFromSource`, measured from the ability's own source ice, which an identity installing out of HQ has none of). 6.2.2b is the third, and 8.5.2d's "unless otherwise indicated" is the sentence it is the indication for; it is one atom per CR clause exactly as the other two are, not a variation on the default. The server is the ATTACKED one and not a named one, because 6.9.4g approaches the attacked server — so that is what "that server" means to a condition met there — and because 4.6.8's remotes are created during play, so no card written before the game can name one; it is the reading `InstallDest::BreachedServerRoot` already takes off the breach. `Instruction::MoveIce` shares the destination vocabulary and so gets the new end too. A mis-citation found in passing is corrected: `MoveIce`'s `InwardFromSource` arm cited 6.2.2b's rule id where the install path cites 6.2.2c's, and both ids stay cited afterwards. **`TriggerRequirement::IceApproachesThisRunAtLeast`**: `GameChange::IceApproached` has recorded every approach since the run structure was written and nothing read it. This is 1.12.6's "this run" read the way `RunnerMadeRun` reads a turn — the change log from the last `RunBegan`, which 10.2.1 makes open information — with the threshold as content (§12 rule 2). What makes 1 the whole of "not the first time" is the ORDER: `MoveRunnerToIce` sets the run's timing step without running it, so the approach the ability causes is not yet recorded when the sentence asks, and the count is exactly the approaches already made. Outside a run — and on 6.1.5b's server, the one with no ice protecting it — it is 0. EDSL: `runner_approaches_a_server()` (6.9.4g, which `TriggerCond::ServerApproached` had carried with no card using it), `move_runner_to_ice(…)` (6.2.8a), `approached_ice_this_run_already()`. Cards: **Mti Mwekundu: Life Improved — COMPLETE**. The printed "you may" sits in the FIRST sentence, and 9.6.9 reads it as the ability's — "such that the ability could potentially have no effects at all" — because both later sentences are stated about what that sentence did. "They may jack out" names the Runner, so it is `performed_by(Runner, may(jack_out()))`, and 1.14.5's peel already in `targets_needed_at` is what puts the yes/no to them; `Instruction::JackOutChoice` is 6.9.4c's structural mirror with no resolver arm and is NOT what an ability uses (the queue's blocker entry said otherwise and was wrong). The third sentence also asks whether the ability installed anything at all (`board_has(&[installed_by_this_ability()], 1)`), because "THIS … time they have approached ice" names no approach when nothing was installed to approach. 3 behaviour tests: the ice going INSIDE the one already protecting HQ with the Runner sent back to it and the offer reaching them before the new approach is recorded; 6.1.5b's case, a server with no ice, where the identity offers nothing to decline; and a Runner who takes the offer, ending the run before the Success Phase with the new ice left installed for the next one. Deleted from the queue's blocker list: "no install destination names the innermost position protecting a server, and nothing counts the approaches of a run" | 247 |
| `-` | W23l | **one identity, and the card an ability INSTALLED when it announced no target.** Shaper 18→19/21 (queue 119→120/150); nothing joins `cr::ANDROMEDA_PILE` — Kabonesa is Shaper, and 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`AbilityFrame::installed_cards`, `TargetFilter::InstalledByThisAbility` and `TargetSpec::InstalledByThisAbility`**: W23h made a 9.6.13 delayed conditional carry the targets its creator ANNOUNCED, which is what lets Arissana's "trash that program" find its card — but an install fed by a search announces nothing at all, since 8.7.4's find is explicitly not 1.15.2's announcement, so "search your stack for a non-virus program and install it … if THAT PROGRAM is still installed" had nothing to read. The card an ability installed is now recorded on the resolving frame at step 8.5.16f, where the installation is complete — so a destination that could not be identified (8.5.14) and a 1.2.2 "cannot install" leave nothing behind — and it is read the way `LookedAtByThisAbility` and `SetAsideByThisAbility` are read, off the innermost ability frame. Deliberately NOT appended to `ability_targets`: that would lie about what was announced and shift every `EarlierTarget` index after it. Like the other 1.15.4 descriptions it fixes cards by IDENTITY, so 1.15.2c's play-area default never applies — which is exactly why the printed "**still** installed" has work to do, and is written as `installed_runner_card()` beside it rather than folded into the word. **`AbilityInstance::bound_installs` + `Payload::DelayedConditional { bound_installs }`** carry it across the delay, the twin of `bound_targets` and threaded through the checkpoint scan beside it. Cards: **Kabonesa Wu: Netspace Thrillseeker — COMPLETE** (9.11.4d is the boundary inside her first sentence: ending the search and shuffling ends an instruction, and the install is the next one, so the printed "then shuffle your stack" is 8.7.3 named in the text rather than a third instruction), 2 behaviour tests — one in which a SECOND program, installed the same turn by the basic action, sits in the rig beside the found one and must survive the turn ending, and one in which the search fails to find (8.7.2e) and the delayed ability must reach nothing at all. Deleted from the queue's blocker list: "nothing names the card an ability installed when the install announced no target". Mti Mwekundu's entry is restated standalone — what it still wants is an innermost install destination and a count of a run's approaches; Saraswati's and Topan's are restated to say what is left of each | 247 |
| `-` | W23k | **one identity, and the cards an occurrence names when it names MORE THAN ONE.** Shaper 17→18/21 (queue 118→119/150); nothing joins `cr::ANDROMEDA_PILE` — Magdalene is Shaper, and 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 70/70. **`GameChange::CardDiscarded` grows `to_hand_size`**: CR 5.7.4's discard step and a card ability's discard (`Instruction::CorpDiscards`) wrote the same record, and "to reach your maximum hand size" is a fact about the MOMENT of the discard — the same kind `CardTrashed::while_accessed` already carries, and recorded there for the same reason, since the step is over by the time any condition is scanned. **`TriggerCond::PlayerDiscardsCards { side, to_hand_size }`** is the condition met by the DISCARD rather than by the phase around it: `DiscardPhaseEnds` is 5.1.4b's formal end of the turn, which happens whether or not a card moved and names none of them, and this one names every card it moved. Per-EVENT (9.12.2a), so a Runner discarding three cards meets it once. **`AbilityInstance::triggering_cards` / `AbilityFrame::triggering_cards` and `TargetFilter::AmongTriggeringCards`** are 1.15.4 in the PLURAL — "those cards", the whole set an occurrence named, where `IsTriggeringCard` reads one. A per-occurrence condition names at most one card and the plural collapses to the singular there; it is a per-event condition that makes the difference, and until now such an instance kept only the FIRST card of its group. Like its singular it fixes cards by identity, so 1.15.2c's play-area default never applies and no zone has to be named beside it. **Latent defect fixed in the same scan**: a per-event condition took its `occurrence_group` from `occurrences[k]` while `k` indexed distinct GROUPS, so with two matching changes in one group the second instance was stamped with the wrong group — out of reach only because no per-event condition had yet been met twice inside one group. The group list is now computed once and indexed by it. Cards: **Magdalene Keino-Chemutai: Cryptarchitect — COMPLETE**, 1 behaviour test in which the program is the SECOND of two cards discarded and the first is an event — a singular reading offers nothing at all — with the declined half asserting the printed "you may". Deleted from the queue's blocker list: "no condition for discarding down to maximum hand size" | 247 |
| `-` | W23j | **one identity, and a strength modification stated about the cards it DESCRIBES.** Haas-Bioroid 15→16/19 (queue 117→118/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can (1.5.4b) — so the deck odometer is unchanged at 70/70. **`StaticDecl::StrengthModMatching { criteria, delta }`** is "all **bioroid** ice has +1 strength": the same 2.5 characteristic modification `StrengthMod` states about the source or its host, stated instead about every card a DESCRIPTION reaches, so the description is content (§12 rule 2) and the class is one declaration — the way `RemoveAbilitiesOfMatching` is one beside `RemoveAbilitiesOf`. It is gathered in `char_effects_of_static` and never announced, because 9.4.1 keeps a static ability from resolving and the described set is re-read on every pass, so a piece of ice that stops being described stops being modified with nothing happening. The reason the sentence waited is deviation 47's loop: the declaration is gathered while the 9.12.1 pipeline is being BUILT, so its criteria go through `filter_matches_shallow` and a subtype in them reads the PRINTED subtypes (2.16) — an ice that merely gained "bioroid" is not described. **Latent defect fixed in the same place: `filter_matches_shallow` did not cover `HasAnySubtype`**, which is the same atom in 2.15's plural and recursed just as surely through `has_subtype`; it was out of reach only because no declaration had yet described its cards by more than one subtype. Cards: **Haas-Bioroid: Stronger Together — COMPLETE**, 1 behaviour test in which two code gates of the SAME printed strength 3 and one Shibboleth of strength 3 decide two runs in opposite directions — 9.3.6c never offers the interface ability against the bioroid one. Deleted from the queue's blocker list: "a declaration cannot modify the strength of cards it DESCRIBES" | 247 |
| `-` | W23i | **one identity, a whole faction, and the FACE an install leaves a card with.** Apex 0→1 (queue 116→117/150); `decks/identities/runner_apex.rs` is new, and 2.13.2's Apex faction is complete at one card. Nothing joins `cr::ANDROMEDA_PILE` — Apex is not Criminal — so the deck odometer is unchanged. **`Instruction::InstallCard` grows `facedown`**: 8.5.16a places the card "with the same faceup or facedown status it will have when the installation is complete", and every install until now took that status from the card's SIDE alone (8.5.2 Corp facedown, 4.6.4c Runner faceup). 4.6.4d is the exception and the installing effect is what states it, so it is content on the install (§12 rule 2) rather than an instruction of its own — `TrashDestination::FacedownInPlay` is 8.1.4d said about a card that is ALREADY installed and was never this. Three things follow from 8.1.4a's "facedown installed Runner cards do not have any characteristics", and each is now read where it is asked: 8.5.11a gives such a card NO INSTALL COST (`install_cost_at` returns 0 before it looks at a card type), the 8.7.2b type gate lifts for a facedown install (a card with no type cannot fail one), and **the memory limit stops counting facedown programs** — `checkpoint.rs` filtered `Zone::Rig` by `card_type == Program` with no regard for the face, which was a latent 1.20.2 defect the moment anything could install facedown. **`StaticDecl::CannotInstallMatching { criteria }`** is "you cannot install <description>" — 1.2.2's "cannot" takes precedence over everything that directs an install, so it is asked at all four places one can be offered or announced (the 5.2.7d action option, an instruction's 1.15.2 candidates, `install_pick_candidates`, 8.7.2b's find) with step 8.5.16a as the backstop for an install whose card was fixed rather than chosen. It is deliberately not a variant of `InstallOnlyHostedOn`, which is 1.13.6c printed on the installee and describes DESTINATIONS; this one is printed on a third card and describes the installee, in the shared filter vocabulary and nothing else. Cards: **Apex: Invasive Predator — COMPLETE**, 3 behaviour tests (an EVENT installed into the rig for nothing, a 5[mu] program that costs no memory, and a basic install action the prohibition removes). Deleted from the queue's blocker list: "a card cannot be INSTALLED facedown". BANGUN's entry is restated standalone — it wanted the other half, and the half it still wants is a DECISION at 8.5.16a rather than a stipulation | 247 |
| `-` | W23h | **five identities, and the number every game is counted towards.** Shaper 16→17, Jinteki 10→13, NBN 16→17 (queue 111→116/150). Arissana Rocha Nahu, Harmony Medtech, Issuaq Adaptics, Nisei Division, Epiphany Analytica; none is Criminal, so `cr::ANDROMEDA_PILE` and the deck odometer are unchanged. **`StaticDecl::AgendaPointsToWinMod { whose, amount }`** and `Vm::agenda_points_to_win`: CR 1.17.2 states winning as a comparison — a score ≥ 7 — and the 7 was a literal in step 10.3.1c. It is now derived, and the declaration modifies THE NUMBER and not any score, so 1.17.1a's threat level and every "agenda points at least" requirement go on reading the real one. `whose` is the same `DeclSubject` scope word `MaxHandSizeMod` carries and `amount` is a quantity position, so "each player needs 1 fewer" and "for each hosted power counter, you need 1 less" are one declaration. Nothing floors it: the requirement is the printed 7 as the declarations leave it. **`TriggerCond::AnyOf { alternatives, requires }`**: 9.6.1a gives an ability ONE primary condition, so a sentence with two is written as two abilities (Leela Patel) — which is wrong the moment the sentence also prints an ordinal, because `AbilityDef::ordinal` belongs to one ability and the pair each spend their own. One condition describing two kinds of occurrence is what the printed "or" says. `trigger_matches` recurses through it, so the readers are taken as `&dyn Fn` at that boundary (a generic recursion instantiates a copy per level and does not compile). An alternative carrying 9.6.5c requirements of its own is a `debug_assert!`: a requirement is checked once against the state and cannot be attributed to one alternative. **`AbilityInstance::bound_targets` + `Payload::DelayedConditional { bound_targets }`**: 1.15.4 across 9.6.13's delay. The frame a delayed conditional resolves in now STARTS with the targets the ability that created it announced, which replaces the ad-hoc `bind_targets` instruction rewrite — so `TargetSpec::EarlierTarget`, `EarlierTargets` and `TriggerRequirement::EarlierTargetMatches` all say the same thing inside a delayed ability as outside one, which is what Arissana's "trash that program **if it is not a trojan**" needs and the rewrite could not reach. **`GameChange::SecretlySpentCreditsRevealed` + `TriggerCond::SecretlySpentCreditsRevealed`**: 10.14.6c's reveal is a step of the psi construction and had no record, so Nisei Division had no occurrence to be met by. Recorded BEFORE the 10.14.4a spends, in the order the rule states — which is why a Corp that bid nothing is still paid. **`TriggerCond::CorpScoresAgenda` grows `criteria`**, `TargetFilter::AdvancedThisTurn(bool)` (1.18.1's advance as the game-history query `InstalledThisTurn` already makes; 1.18.2's bare counter placement is not one), and **`TimingRestriction::RunOnly`** (9.3.3c's limit on WHEN, naming the run structure rather than one of its phases). Four blocker entries retired, two rewritten to say exactly what is left. Workspace green, `nix build .#default` verified on the staged tree. | 247/247 |
| `-` | W23g | **two identities, and the two words a run and a pile were missing.** Criminal 20→21, Anarch 13→14 (queue 109→111/150); Nero Severn is complete, so it joins `cr::ANDROMEDA_PILE` and the priority-deck odometer grows to 70 with both decks still playable. **`Instruction::JackOut`** is CR 6.1.5's PROCESS — the Runner voluntarily ending a run — as distinct from `Instruction::JackOutChoice`, which is 6.1.5b's STEP: the step is an opportunity the run's structure opens after passing ice, and a card that OFFERS the choice somewhere else (Nero Severn, during an encounter) had nothing to denote into. 6.1.5's own words are that jacking out "follows the usual process for ending the run", so it makes the same `EffectClass::EndTheRun` atom imminent — an ability relevant to a run ending stays relevant to it (9.9.3a) — and it ends the run by the same path the step's yes-branch takes. 6.1.5's remaining sentence (some abilities function differently depending on whether the Runner CHOSE to end the run this way) has no reader in the kernel, and the step records the choice no more than this does. **`StaticDecl::DiscardPileIsOrdered { whose }` and `TargetSpec::TopOfDiscard`** are one word in two halves: CR 4.4.2 is that discard piles are NOT ordered — "a player may freely arrange the cards in their discard pile in any order at any time" — so a heap has no top card at all, and Wyvern's "you must maintain the order of your heap" is the declaration that takes that freedom away and gives the next sentence something to name. `TopOfDiscard` therefore reads the declaration and describes NOTHING without one (9.1.4's stranded reference), which is what keeps it from being a word every card could quietly use. Also **`TriggerCond::RunnerTrashesCorpCard` grows `requires`**, so the draft-format faction-partition clause rides inside the condition (9.6.5c) as it does on every other trigger. **Six blockers recorded, none approximated**: AgInfusion (a choice BETWEEN servers has no branches to write while 4.6.8's remotes are created during play), Ob Superheavy (a trash records neither that the card was rezzed nor that it happened during an installation — both facts about the moment, the way `while_accessed` already is — and "ignoring **credit** costs" cuts across 1.16.4's inherent/additional split that `ignore_costs` follows), Freedom Khumalo (a counter cost is a printed number off the SOURCE, not an announced X taken from any of the payer's cards), Adam (nothing installs a card at setup), Skorpios (a trash cannot be replaced by SETTING ASIDE), and Mti Mwekundu (joins the existing "nothing names the card an ability installed" entry). | 247/247 |
| `-` | W23f | **four identities, and the defect a reaction to a PAYMENT was hiding.** NBN 12→15, Weyland 11→12 (queue 100→104/150). **`Instruction::GainTags` grows `avoidable`** — 9.3.3g makes "(cannot be avoided)" a RESTRICTION and 9.4.5 makes a restriction ride the value, so it is the same `EffectAtom::unpreventable` flag `DamageUnpreventable` already sets for the damage half of that sentence pattern, and 9.9.3a then finds no avoider relevant to the tag (NBN: Controlling the Message). **`TriggerCond::TagRemoved`** is 10.5.1's counter going back to the bank, met once per tag and by either player's removal — the counterpart of `RunnerTakesTag`; **`Cost::remove_tags`** is the same movement as a COST ("[click], remove 1 tag:"), unpayable for want of tags rather than for an interrupt (1.16.1b), and 1.16.10b records it where the condition can meet it, which is how Synapse Global's two printed lines feed each other. **`GameChange::{CreditsLost, CostPaid}` gain `source`** — the field `CreditsGained` has carried since W23e, read the same way (9.1.4) — so `TriggerCond::PlayerPaysCredits` can grow `also_lost` (1.10.3b's forced loss and 1.10.3c's payment as ONE condition, the shape `PlayerSpendsClick` already has for clicks), `caused_by` ("a **Corp card ability** causes", asked of the source in the shared filter vocabulary) and 9.6.5c's `requires` ("during a run") — GameNET, whole. Also **`Instruction::InstallCard` reachable with `ignore_costs` from the card layer** (1.16.5c, which removes 8.5.11a's 1[credit] per ice already protecting the destination). **Defect fixed: a nested cost was put to the payer AGAIN for every reaction its own payment triggered.** 9.11.4f ends the instruction with the CHOICE, but the frame left the announcement phase only after `pay_cost` returned — and 1.16.3's payment carries a 10.3.4 checkpoint that can open a reaction window and push a frame, after which `frames.last_mut()` was some other frame and the phase assignment silently did nothing. Closing the window returned to an ability still waiting to announce, so the same "pay 3[credit] or end the run" was charged until the Runner ran out of credits. The phase now moves before anything is paid, and 1.16.11's follow-on effect is queued before the payment for the same reason. Cards: **SSO Industries, NBN: Controlling the Message, GameNET, Synapse Global — all COMPLETE**, 4 behaviour tests. Skipped with reasons recorded on the queue: A Teia, AU Co., AgInfusion, BANGUN, Chronos Protocol (Selective Mind-mapping), Mti Mwekundu, Nuvem SA, Ob Superheavy, Saraswati Mnemonics, Skorpios. No card joined a pile: the only pile is the Runner's (1.5.4b), so `priority_decks()` is 68 cards unchanged | 247 |
...
28. **`MoveIce` accepts the `InstallDest` vocabulary** (W8a, extended W23m) —
    6.2.2's position language is the same one 8.5.16b uses, so `MoveIce`
    reuses it and implements the three arms that name a position protecting a
    server (`Protecting` = 6.2.2a outermost, `InwardFromSource` = 6.2.2c's
    directly-inward, `InnermostProtectingAttackedServer` = 6.2.2b's innermost
    end, on the server 6.1.2 names). Any other destination moves nothing.
    6.2.2d's "in any position" (a Corp choice among the gaps) and 6.2.2e's
    Mutate case are not expressible yet.
...
47. **`RemoveAbilitiesOfMatching` reads its criteria shallowly** (W11b) —
    ...
    criterion there reads printed subtypes. Since W23j the same is true of
    `GainAbilitiesOf`, `GainSubtypesOf` and `StrengthModMatching`, and of
    `HasAnySubtype` as well as `HasSubtype` — every criterion naming a
    subtype, in every declaration gathered by that pass.
...
55. **The damage selection is announced in ONE decision, not one per point**
    (W12e, narrowed W23o) — 10.4.3a has the cards "selected sequentially", and
    the kernel asks the selecting player for all of them at once through a
    single `ChooseTargets`. Nothing may happen between two of those
    selections, so the order they are named in is not observable. *(The rest
    of this entry is RETIRED (W23o) — "`SelectsDamageTrashes` has a count, not
    an ordinal", the once-per-turn limit elided. The declaration now carries
    the damage type, the printed ordinal and the printed "you may", so Chronos
    Protocol: Selective Mind-mapping and Titanium Ribs no longer differ only
    by `count`.)*
...
76. **Characteristic criteria in 9.6.5c's ordinal re-reads go through the
    CURRENT 9.12.1 pipeline** (W23y, the residue after the moment-facts moved
    onto their records) — the rescan's `matches_criteria` answers a subtype
    or type stipulation with the pipeline as it stands at the RE-read, which
    is exact for printed characteristics (they travel with the card) and
    wrong for one granted with a duration: an ice that was a code gate at its
    encounter's end (Kit's grant) and is not one now would be un-counted, and
    the other direction over-counted. Unhit: the one ordinal condition that
    stipulates a subtype asks for "Bioroid" (Architects of Tomorrow), and no
    corpus effect grants that subtype — Kit grants "code gate", which no
    ordinal reads. The fix when the two meet is the same record-vs-state
    move W23y made three times.
    `TriggerCond::CorpScoresAgenda`'s `criteria` sit in the same class, and
    its one non-characteristic filter (`AdvancedThisTurn`) is a game-history
    query and already answers for the past.
77. **A tag reader reached from INSIDE the considered-tag gather answers
    with the REAL count** (W23z, the `considered_tags_gathering` latch in
    `Vm::considered_runner_tags`) — deviation 2b's loop in the tag pipeline,
    broken dynamically where 2b's is broken statically, because this loop
    has two mouths and one of them is a quantity: the gather of
    `StaticDecl::ConsideredTagsMod` asks every carrying static ability's
    9.3.7a stated condition, which can BE `RunnerTagsAtLeast` (Harishchandra
    Ent.), and its `abilities_of` walk runs the 9.12.1 pipeline, whose own
    gather evaluates `SelfStrength` quantities that can be
    `Quantity::RunnerTags` (Resistor — strength +1 per tag). A static
    per-atom shallow match (2b's exact shape) would have had to shadow
    `eval_quantity` whole, so the break is a latch instead: while the
    gather holds it, `considered_runner_tags` answers `st.runner.tags`.
    The observable residue is one level deep, exactly 2b's: OUTSIDE the
    gather every reader sees the modified number — Harishchandra + Acme
    compose (the grip opens during the qualifying encounter), Resistor
    grows during it — but a stated condition or strength read WHILE the
    gather is asking reads the real count, so a hypothetical
    ConsideredTagsMod GATED on "the Runner is tagged" cannot be lifted by
    another one's considered tag; no corpus card states one.
...
- Commit per coherent sub-wave; hand off by updating this file when context
  nears its end.
| `-` | W23aa | **one identity, and the moment a trash remembers.** Weyland Consortium 15→16/19 (queue 132→133/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can (1.5.4b) — so the deck odometer is unchanged at 71/71. Three kernel words, one per printed clause. **`GameChange::CardTrashed` gains `was_rezzed` and `during_install`** — 8.1.2's "a **rezzed** card" (a faceup installed Corp card, snapshotted BEFORE the move because 10.3.1a turns a Corp-trashed card facedown) and 8.5.11a's like-card trash (the one "**except during installation**" excludes, read as `!installs.is_empty()` — installing is a procedure with no timing structure per 9.2.2e, so no ability-driven trash can run mid-install in this kernel; the doc comment records the approximation honestly); `TriggerCond::CardTrashed` gains `rezzed_only`/`except_during_install` and compares the record (the A Teia record-vs-state rule — the state no longer says either). **`TargetFilter::RezCostRelativeToTriggeringCard { delta }`** — "a printed rez cost exactly 1[credit] less than the trashed card's" is a RELATIONAL atom beside `SameNameAsTriggeringCard`, comparing 2.3's printed number the way that one compares the name; both sides must BE cards that rez (8.1.2's assets/ice/upgrades) and HAVE a printed cost, so a 2-cost operation in R&D matches nothing, and the trashed card still answers from the discard pile because printed numbers travel with it. **`Instruction::InstallCard` gains `ignore_credit_costs`** — a different axis from `ignore_costs` (1.16.5c as this kernel reads it: inherent-only, the Ob/Archer case in `InstallRezPayCost`): "ignoring **credit** costs" selects components by KIND and cuts across 1.16.4's inherent/additional split — inherent install cost, rez cost and the credit part of an additional cost all 0 via `Cost::without_credits`, an Archer-class forfeit STILL paid (and an additional cost that WAS only credits leaves no payment to put to the Corp at all); threaded through `InstallProgress`, `imminent_cost_credits`, `pay_install_cost`, the `InstallRezPayCost` arm, `FollowOn::Install` and `could_install_found_card` (8.7.2b: install-side costs are credits and nothing else, so under ignore-credit-costs every candidate is findable; the rez side never gated findability — the Tucana reveal). **Defect fixed en route (9.3.6g): two occurrences of one checkpoint create two pending instances BEFORE either is used, and the second stayed offered after the first spent the flag** — `reaction_options`/`interrupt_options` now drop an optional instance whose `(ability, generation)` is in `once_per_turn_used` (a mandatory instance is never "used", 9.1.6). EDSL: `trashes_a_rezzed_card_except_during_install(by)`, `rez_cost_exactly_less_than_the_triggering_cards(n)`, `install_and_rez_found_ignoring_credit_costs()`. Cards: **Ob Superheavy Logistics: Extract. Export. Excel. — COMPLETE** (`may_when_once_per_turn`, the Mercury flag+optional shape), 5 behaviour tests — the happy path at 0[credit] the whole way (offer, find excludes same-cost asset AND 2-cost operation, installed AND rezzed, pool untouched, `was_rezzed`/`during_install` asserted on the record); an unrezzed trash bringing no offer; the 8.5.16c like-card trash of a fixed-destination install bringing none (`during_install: true` on the record); one instruction trashing TWO rezzed assets and the flag spending once (the 9.3.6g fix's test — offers == 1); an R&D with no matching rez cost completing the search empty, 8.7.3 still shuffling, nothing installed. Deleted from the queue's blocker list: "a trash records neither that the card was REZZED nor that it happened during an installation" | 247 |
## Odometers (after W23ac)
- **DP-7b: 895/1420** distinct rules cited (63.0%, the traceability test's
- **Identity queue: 133/150** (`docs/vm/IDENTITY-QUEUE.md`, the list and the
  a pile. Runner Criminal 22/22 (COMPLETE), Shaper 19/21, Anarch 16/19,
  Jinteki 18/21, NBN 17/19, Weyland 16/19, Neutral 3/4. **Every complete
| `-` | W23ac | **one identity, and a trash replaced by SETTING THE CARDS ASIDE — as one group.** Weyland 15→16/19 (queue 132→133/150); nothing joins `cr::ANDROMEDA_PILE` — a Corp identity never can (1.5.4b) — so the deck odometer is unchanged at 71/71. **`StaticDecl::SetsTrashedCardsAside { criteria, then, until_removed_with_it_this_turn }`** is 9.9.8b's replacement of the trash movement said about "1 or more cards" as ONE GROUP — `ReplaceTrashDestination`'s sibling, read in `trash_card` for the same reason (10.4.2's damage trashes are not instructions and 1.16.1a's cost trashes cannot be interrupted, yet both are movements a replacement reaches), except that the destination is 4.8's set-aside zone: every matching card of one occurrence joins the same 4.8.7 group (`Vm::pending_trash_set_aside`, gathered until the occurrence's own checkpoint), faceup per 4.8.6, with `CardTrashed` recorded at the interception (8.2.2: still an occurrence of trashing) and no `CardMoved` at all — 4.8.3 makes an ability that never names the set-aside zone see ONE movement, reported from the card's real location when the group completes, the shape 8.4.5a's drawn set-aside already has. The declaration's own remaining instructions (`then`) resolve ONCE over the group as a mandatory instance minted into 9.6.14d's pending-from-effect path at the checkpoint (`Vm::pend_trash_set_aside_group`, the `pend_if_successful` shape for a clause a DECLARATION carries); **`AbilityInstance` gains `set_aside_group`**, copied onto the frame, so `TargetFilter::SetAsideByThisAbility` and the new **`TargetSpec::StillSetAsideByThisAbility`** ("all of those cards that are still set aside" — no announcement, the `FoundBySearch` shape; "still" is the zone) read the group exactly as they read one the frame's own instruction set aside. **`Instruction::AddCardsToHeap`** is the movement completing — "add … to the heap" is 4.4.1's word fixing the destination, an ADD and not a second trash. **The per-turn ignore is spent by the REMOVAL, not by the interception**: 9.4.1's statics never resolve, so 9.3.6g's use-spent flag cannot be what "Ignore this ability if you have already removed a card from the game with it this turn" means — **`GameChange::CardsRemovedFromGame { objs, by_ability_of }`** is the `RemoveCardsFromGame` instruction's own record (the `CardInstalled::by_ability_of` attribution shape), and the declaration stops applying while a non-empty one attributed to its card sits in the turn's log (10.2.1; the `SelectsDamageTrashes::first_each_turn` read). A group set aside before the removal still completes — its follow-up is the movement finishing, not a new application. One visibility repair rode along: a FACEUP set-aside group is visible to both players (`o.faceup || g.by == side`) — 4.8.6's default-faceup was unreadable through the arm that hid every group from the non-owner, a privacy 8.3.3a states about FACEDOWN arrange-groups only. EDSL: `set_trashed_aside_then_until_removed_with_it_this_turn`, `still_set_aside_by_this_ability()`, `set_aside_by_this_ability()`, `add_to_heap`. Cards: **Skorpios Defense Systems: Persuasive Power — COMPLETE**, 5 behaviour tests — a single Corp-effect trash set aside and removed (heap empty, the trash still recorded, the set-aside zone empty after); a 2-point net damage intercepted ONCE as one group (one removal announcement, the survivor to the heap, `DamageSuffered` carrying both); the decline sending the whole group to the heap together; the ignore across turns (a removal, then a second trash the same turn straight to the heap with nothing asked, and the next turn's trash intercepted again); a Corp card's trash untouched. Each of the five widenings (the ignore-scan, the group-joining, the instance→frame group copy, the faceup-group visibility, the removal's attribution) was reverted in turn to confirm a test fails without it. Deleted from the queue's blocker list: "a trash cannot be replaced by SETTING THE CARDS ASIDE" | 247 |
| `-` | W23ad | **three flip identities, and the three small words their back faces waited on.** Anarch 16→17/19, Shaper 19→20/21, NBN 17→18/19 (queue 133→136/150 — the 132→133 step was Earth Station, ticked by the back-face-data wave `c442d78`, which wrote no row here); nothing joins `cr::ANDROMEDA_PILE` — Hoshiko is Anarch, Dewi is Shaper, SYNC is Corp, and 1.5.4b makes the only pile in the priority decks Criminal — so the deck odometer is unchanged at 71/71. **`Quantity::AccessesThisTurn`** is 7.3.6's actually-performed count over 1.12.6's TURN window instead of the run's — `CardAccessed` is recorded at the one step an access is actually performed (`StepKind::CardBecomesAccessed`), so counting those records since `turn_log_start` is the whole reader, and a replaced access left nothing to count. Hoshiko's two faces are the two directions of the one amount (§12 rule 2): "accessed a card this turn" is at-least-1, "did not access any cards this turn" is at-most-0, both asked at the turn-end occurrence (9.6.5c). Her back's morning is one combined sentence, and 1.10.3b is why "lose 1[credit]" at 0 loses nothing and still draws. **`Quantity::UnusedMemory`** is CR 1.20.4a's OWN calculated value — the memory limit (1.20.2, as modified) minus installed programs' memory costs (1.20.3), counted as the checkpoint counts them (8.1.4a: a facedown card in the rig has no memory cost) — so Dewi's "[mu] is full" is at-most-0 unused and her back's "at least 1 unused" is at-least-1, on `MakesSuccessfulRun`'s existing 9.6.5c `requires`. **`StaticDecl::BasicActionCostMod { action, amount }`** is 1.16.2 said about the credit part of ONE basic action's printed cost — the "(not through a card ability)" parenthetical is 5.2.5a's action identity, so the declaration names `change::BasicAction` and its reader `Vm::basic_action_credits` lives at the four places 5.2.7g's remove-tag and 5.2.6g's trash-resource count and pay their credits, floored at 0 (1.16.2a); SYNC's front is +1 on remove-tag, its back −2 on trash-resource, and the back's "may pay fewer" keeps to `Vm::install_payment`'s affordable-anyway KERNEL APPROXIMATION precedent (the choice to overpay is not offered). SYNC's back face keeps the front's name — its `faces[0].title` is null in the source data — and both faces print the [click]-flip. Cards: **Hoshiko Shiro: Untold Protagonist, Dewi Subrotoputri: Pedagogical Dhalang, SYNC: Everything, Everywhere — all COMPLETE, both faces**, 12 behaviour tests | 247 |
| `-` | C1 | **the game-start cluster: §1.6 grows its three ability points, and NEXT Design, Ayla and Adam land whole.** Shaper 19→20/21, Haas-Bioroid 17→18/19, Adam 0→1/1 (queue 133→136/150); nothing joins `cr::ANDROMEDA_PILE` — Ayla and Adam are not Criminal, NEXT is Corp — so the deck odometer is unchanged. **`SetupPhase` grows `BeforeStartingHands` and `BeforeFirstTurn`**, and §1.6 stays a PROCEDURE, not a timing structure (9.2.2 lists the timing structures and setup is not one): the two points are phases of the toplevel pump holding `Vm::setup_queue`, each resolved as an ordinary conditional-resolution frame, Corp first (1.6.2). `new_game` defers the 1.6.6 draw behind the first point only when an identity actually prints a pre-draw ability, so every existing game returns with hands drawn exactly as before. **`TriggerCond::{BeforeDrawingStartingHand, BeforeTakingFirstTurn}`** are 1.6.1a's setup alteration at the step the text indicates and 1.6.7a's pre-first-turn resolution — met by NO change record ever; the setup procedure resolves them directly, which is what makes a mulligan unable to re-meet Ayla's (1.6.6a shuffles the STARTING HAND back; the set-aside cards are in neither the hand nor the stack, and the redraw is the mulligan's own procedure, not a second 1.6.6). **`PrintedCard::starting_extra_installs` + `GameSetup::extra_cards`** are 1.5.3 as the fourth setup FACT of the `starting_*` family: the directives are brought from outside the deck (1.5.3a, `Zone::OutsideGame`), exactly N differently-named matching ones begin the game installed at 1.6.2 (1.5.3b — a state the game is built in: no 8.5 procedure, no cost, no record a "when you install" condition could meet), ordinary installed cards from then on (1.5.3d — Safety First's −2 is in force before the first mulligan decision). **`SetAsideGroup.with` + `SetAsideTopOfDeck::with_source` + `TargetFilter::SetAsideWithSource`** — 4.8.7's group stamped with the CARD whose ability set it aside, so a LATER ability of the same card reaches it after the frame is gone (4.8.3's "refers to set-aside objects", the other way round); the "(You may look at those cards at any time.)" entitlement is the facedown group's existing visibility (`by == side`), no new peeking model needed. **`Instruction::Draw` becomes a quantity position** (§12 rule 6) with `Quantity::CardsInHandOf` — "Draw until you have 5 cards in HQ" is `Const(5) − CardsInHandOf`, evaluated at resolution, floored at 0; a short R&D gives what it has, because 1.7.2c's "required to draw" is 5.3's mandatory draw and this is not it (the behaviour test pins the contrast: the first turn formally begins on the empty R&D, and THEN the mandatory draw flatlines). **`InstallCards`/`InstallCard::distinct_servers`** — "with no more than a single piece of ice per server" excludes a server this ability already installed to from the 8.5.16b offer (read off `AbilityFrame::installed_cards`); costs stay paid per 8.5.11, and under the stipulation each install is a server's first ice of the effect, so the payment is always 0. Riders: `Quantity::MaxHandSizeOf` + `TriggerRequirement::QuantityLessThan` (Safety First's turn-end draw), `TriggerCond::RunnerAccessesCard` grows `criteria` + `TargetFilter::HasTrashCost` (Neutralize All Threats' first line, with `MustTrashAccessedCard`/`PayingTheTrashCost` waiting since W-whenever), and Adam's directives ride `decks/identities/runner_adam.rs` (deck-odometer definitions 198→204). Cards: **NEXT Design: Guarding the Net, Ayla "Bios" Rahim: Simulant Specialist, Adam: Compulsive Hacker — COMPLETE**; **Neutralize All Threats, Safety First — COMPLETE**; **Always Be Running** honestly partial (its first-click action restriction is a 5.2 offer-narrowing declaration that does not exist; its second line is whole — `Cost::lose_clicks` existed since W7e). 8 behaviour tests: the three ice one-per-server with the second declaration not offering the used server, decline-all with zero draws, the short-R&D contrast, Ayla's six-aside/two-back before any hand with the click retrieval and the mulligan invariance, and Adam's three directives installed-and-active (max hand size already 3) with the stack untouched. Deleted from the queue's blocker list: "Nothing happens BEFORE the first turn" and "Nothing installs a card at the game's SETUP" — Cyber Bureau still waits on upstream face data and Biotech on the N-faces word, both recorded in the queue's data-gap section | 247 |
