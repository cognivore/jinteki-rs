...
exists in the kernel as `Instruction::ForEach { count, effects }`, and — an
earlier wave said otherwise — it IS reachable from the card vocabulary, since
`Instruction` is among `crate::edsl`'s re-exports. What it has no value to
give is `count`, so it lands with the quantity below and not before it. Note
the printed arithmetic: "do this, then repeat for each X" is 1 + N
resolutions, never N.)
